...
window.onload = function () {
    ...
    if (currentUrl.toLowerCase().includes("/discuss")) {
        ...
        let contactNums = document.getElementsByClassName("contact-number-text");
        ...
        if (contactNums.length > 0) {
            for (let i = 0; i < contactNums.length; i++) {
                let countryName = detectCountryFromPhone(contactNums[i].innerHTML);
                if (countryName != null) $("#" + contactNums[i].id).html(contactNums[i].innerHTML + " ∙ <span class='text-muted'>" + countryName + "</span>");
                else $("#" + contactNums[i].id).html(contactNums[i].innerHTML + " ∙ <span class='text-muted'>unknown country</span>");
            }
        }
    }
...
function textDesigner(value) {
    ...
    if (value.includes(":")) {
        ...
        let countryNameIfItExiststInText = detectCountryFromPhone(value);
        if (countryNameIfItExiststInText != null) return '<div class="box-container bg-white p-2"><div class="image-picture-none-container-sm text-center p-1 d-inline-block"> <i class="fa-regular fa-address-card text-primary"> </i> </div><span class="h5 ms-2 text-dark">' + contactName + '</span><div class="mt-1"></div><small class="card-text fw-500 text-dark">' + contactDetails + ' ∙ <span class="text-muted">' + countryNameIfItExiststInText +'</span><div class="border-top pt-1 mt-1"><button type="button" class="btn btn-standard-asset btn-light btn-sm text-primary me-1"> <i class="fa-regular fa-copy"></i> Copy</button><button type="button" class="btn btn-standard-asset btn-light btn-sm text-primary"> <i class="fa-solid fa-phone"></i> Call</button></div></div> ';
        else return '<div class="box-container bg-white p-2"><div class="image-picture-none-container-sm text-center p-1 d-inline-block"> <i class="fa-regular fa-address-card text-primary"> </i> </div><span class="h5 ms-2 text-dark">' + contactName + '</span><div class="mt-1"></div><small class="card-text fw-500 text-dark">' + contactDetails + ' ∙ <span class="text-muted">unknown country</span></small><div class="border-top pt-1 mt-1"><button type="button" class="btn btn-standard-asset btn-light btn-sm text-primary me-1"> <i class="fa-regular fa-copy"></i> Copy</button><button type="button" class="btn btn-standard-asset btn-light btn-sm text-primary"> <i class="fa-solid fa-phone"></i> Call</button></div></div>';
    }
...
function detectCountryFromPhone(text) {
    let indexOfPlus = text.indexOf("+");
    if (indexOfPlus >= 0) {
        let nextSpaceIndex = text.indexOf(" ", indexOfPlus);
        nextSpaceIndex = nextSpaceIndex <= 0 ? text.length : nextSpaceIndex;
        let phoneFullNumber = text.substring(indexOfPlus, nextSpaceIndex);

        for (let i = 0; i < countriesAndCodes.length; i++) {
            if (phoneFullNumber.includes(countriesAndCodes[i].code)) {
                return countriesAndCodes[i].name;
                break;
            }
        }
    }
