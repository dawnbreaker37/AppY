...
namespace AppY.Controllers
{
    public class AccountController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> SendOneTimeCode(string Email)
        {
            if(ModelState.IsValid)
            {
                ...
                if(OneTimeCode != null)
                {
                    ...
                    if(Result)
                    {
                        ...
                        return Json(new { success = true, alert = "Single-use code has just been sent to your email. Please, check your email and enter that code to make it clear that you're the owner of this account. <br/>The sent code will be active within <span class='fw-500 text-neon-purple'>12</span> mins " });
                    }
                }
            }
        ...
        {
            bool Result = await _user.IsEmailUnique(Email);
        ...
        {
            bool Result = await _user.IsEmailUnique(Username);
